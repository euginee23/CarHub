{{-- Self-contained error shell.
     ...
     Deliberately does NOT use partials/marketing-head, the marketing nav/footer,
     Livewire, or Flux: an error page has to render when the app is unhealthy. The
     @vite call is guarded because a fresh clone with no built assets would
     otherwise throw a ViteException *from the error page itself*, and links use
     url() rather than route() so a routing fault cannot cascade. --}}
@props([
    'code',
    'title',
    'message' => null,
...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ $code }} &middot; {{ $title }} - {{ config('app.name', 'CarHub') }}</title>
    ...
    <body class="min-h-screen bg-white text-zinc-900 antialiased">
        <div class="relative isolate flex min-h-screen flex-col overflow-hidden">
            ...
            <div aria-hidden="true" class="pointer-events-none absolute inset-0 overflow-hidden">
                <div class="absolute -left-32 -top-40 size-[36rem] rounded-full bg-brand-400/20 blur-3xl"></div>
                <div class="absolute -right-24 -top-24 size-[30rem] rounded-full bg-spark-400/15 blur-3xl"></div>
            </div>
            ...
            <header class="relative px-4 py-6 sm:px-6 lg:px-8">
                <a href="{{ url('/') }}" class="inline-flex items-center gap-2.5">
                    ...
                    <span class="text-lg font-bold tracking-tight text-zinc-900">{{ config('app.name', 'CarHub') }}</span>
                </a>
            </header>
            ...
            <footer class="relative px-4 py-8 text-center sm:px-6 lg:px-8">
                <p class="text-sm text-zinc-500">
                    &copy; {{ now()->year }} {{ config('app.name', 'CarHub') }}
                </p>
            </footer>
        </div>
    </body>
